 ...
 */
public class GearTest5 extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        int centerX = 150;
        int centerY = 125;
        paintGear(g2d, centerX-radius, centerY-radius, radius*2, 30, teethSize, offsetAngle, Color.PINK);

        gearAngle = 45;
        int x = centerX + (int)((radius*2 - teethSize)*Math.cos(Math.toRadians(gearAngle)));
        int y = centerY + (int)((radius*2 - teethSize)*Math.sin(Math.toRadians(gearAngle)));
        paintGear(g2d, x-radius, y-radius, radius*2, 30, teethSize, -offsetAngle, Color.RED);
        
        gearAngle = 120;
        x = centerX + (int)((radius*2 - teethSize)*Math.cos(Math.toRadians(gearAngle)));
        y = centerY + (int)((radius*2 - teethSize)*Math.sin(Math.toRadians(gearAngle)));
        paintGear(g2d, x-radius, y-radius, radius*2, 30, teethSize, -offsetAngle + 6, Color.GREEN);
        
        gearAngle = 270 + 60;
        x = centerX + (int)((radius + radius/2 - teethSize)*Math.cos(Math.toRadians(gearAngle)));
        y = centerY + (int)((radius + radius/2 - teethSize)*Math.sin(Math.toRadians(gearAngle)));
        paintGear(g2d, x-radius/2, y-radius/2, radius, 15, teethSize, -offsetAngle*2 + 12, Color.BLUE);
    }
    
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            final GearTest5 panel = new GearTest5();
