 ...
 */
public class GearTest4 extends JPanel {
    private static final boolean animationEnabled = false;
    ...
    protected void paintComponent(Graphics g) {
        ...
        int x = centerX + (int)((radius*2 - teethSize) * Math.cos(Math.toRadians(45)));
        int y = centerY + (int)((radius*2 - teethSize) * Math.sin(Math.toRadians(45)));
        paintGear(g2d, x-radius, y-radius, radius*2, 30, teethSize, -offsetAngle, Color.RED);
    }
    
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            final GearTest4 panel = new GearTest4();
